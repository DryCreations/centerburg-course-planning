# TUESDAY (Sep 15): Slide Prompts (all classes)

Every block is **one prompt, for one slide.** Each class also has a **Board version**: the same content
reduced to what fits on a whiteboard, for running the lesson without a projector.
...
## 4. Middle School CS: `for index` and modulo

**Documents:** `handouts/star-catcher-lab.md`, steps 6 to 9
**Run it as:** content first, both concepts with example problems, then lab steps 6 to 9.

### Slide 1: As You Come In

> Create one "As You Come In" slide for a middle school computer science class. Big title: "As You Come In."
> Body: "Open Star Catcher and run it. Your stars should be falling from random spots at the top. Today we
> make them show up in a PATTERN instead, and we do it with a loop that can count." Large, clean, minimal. One
> slide.

### Slide 2: repeat cannot count

> Create one slide for a middle school computer science class titled "repeat Has a Problem." Body in large
> text: "A repeat loop runs your code ten times, but it has NO IDEA which time it is on. Every pass is
> identical. So if you want ten stars in ten different places, repeat cannot help you. You need a loop that
> counts." One slide.

### Slide 3: for index

> Create one slide for a middle school computer science class titled "for index: The Counting Loop." Show in a
> monospace box: "for index from 0 to 4:" then indented "set star to sprite of kind Food" and "set star
> position to x (20 + index * 30), y 0". Underneath, a trace table with columns Pass, index, and the x value:
> pass 1 index 0 gives 20; pass 2 index 1 gives 50; pass 3 index 2 gives 80; pass 4 index 3 gives 110; pass 5
> index 4 gives 140. Add a box in large text: "for index from 0 to 4 runs FIVE times. It counts 0, 1, 2, 3, 4.
> This catches everybody once." One slide.

### Slide 4: Modulo, the remainder block

> Create one slide for a middle school computer science class titled "remainder: What Is Left Over." Body: "In
> the Math category there is a block: remainder of ( ) divided by ( ). It gives you what is LEFT OVER after
> dividing." Then a small table: remainder of 7 ÷ 3 is 1, because 3 goes into 7 twice with 1 left; remainder of
> 12 ÷ 5 is 2; remainder of 10 ÷ 5 is 0, it divides evenly; remainder of 9 ÷ 3 is 0. One slide, large readable
> text.

### Slide 5: Every Nth

> Create one slide for a middle school computer science class titled "That Is How You Do Every Nth." Body in
> large text: "remainder of N ÷ 5 = 0 is true EXACTLY when N is a multiple of 5. So it is true at 0, 5, 10,
> 15, 20." Then a table: every 2nd means remainder of N ÷ 2 = 0; every 3rd means ÷ 3 = 0; every 5th means ÷ 5 =
> 0; every 10th means ÷ 10 = 0. Add a box: "Common mistake: writing '= 5' instead of '= 0'. A remainder can
> never equal the number you divided by." One slide.

*(Problems to ask aloud: what is remainder of 14 ÷ 4? (2) For which of 1 to 10 is remainder of N ÷ 3 = 0? (3,
6, 9) What condition gives a bonus every 4th star? (remainder of score ÷ 4 = 0))*

### Slide 6: Do Now

> Create one "Do Now" slide for a middle school computer science class. Big title: "Do Now: Steps 6 to 9."
> Body: "In the lab: 1) Replace your repeat with 'for index from 0 to 4'. 2) Set the star's x to 20 plus index
> times 30, so you get an even row instead of random spots. 3) Set its vy to 30 plus index times 10, so each
> one falls faster than the last. 4) Add a starCount variable and an if using 'remainder of starCount ÷ 5 = 0'
> to make every fifth star a fast, different-colored one." Add a box: "Finished? Make a row that goes DOWN the
> side instead of across the top. What do you change?" One slide.

### Board version (no projector)

```
repeat  = runs N times, has NO IDEA which pass it is on
for     = runs N times AND tells you the number

  for index from 0 to 4:
      set star x to (20 + index * 30)

  pass  index   x
   1      0     20        <- FIVE passes, not four.
   2      1     50           it counts 0,1,2,3,4
   3      2     80
   4      3    110       remainder = what is LEFT OVER
   5      4    140         7 / 3 -> 1
                           12 / 5 -> 2
                           10 / 5 -> 0   <- divides evenly

  remainder of N / 5 = 0  is true at 0, 5, 10, 15...
  THAT is how you do "every 5th"
  (common error: writing = 5 instead of = 0)
```

---

## 5. Yearbook: Shutter speed

**Documents:** `handouts/shutter-speed.md`
**Run it as:** the lesson with example photos, then walk the four shot set, then rotation and spread work.

### Slide 1: How shutter speed works

> Create one slide for a high school yearbook class titled "Shutter Speed." Three numbered points in large
> text: 1) Shutter speed is HOW LONG the sensor is exposed to light, written as a fraction of a second. 2)
> Bigger bottom number means FASTER means MORE FROZEN. 1/1000 is much faster than 1/60. 3) Faster shutter
> means DARKER photo, because less light gets in. That is the trade, and it is why you then open the aperture
> or raise the ISO. Then a short reference table of situations and starting points: indoor sports 1/500 or
> faster; outdoor sports 1/1000; someone walking 1/250; posed portrait 1/125; never below 1/60 handheld;
> deliberate blur 1/30 or slower with the camera braced; panning about 1/60. One slide.

### Slide 2: The assignment

> Create one slide for a high school yearbook class titled "The Shutter Speed Set: Four Shots." A numbered
> list, each with its target: 1) FROZEN, 1/500 or faster, someone clearly moving stopped dead with no smear.
> 2) PANNING INDOORS, about 1/60, move the camera WITH the subject so the subject is sharp and the background
> streaks. 3) WATER FROZEN, 1/1000 or faster, individual droplets in mid-air instead of a stream. 4) SUBJECT
> BLURRED, 1/30 or slower with the camera braced on something solid, so the background is sharp and the moving
> person is a streak. Then, in a box: "For each: note the actual shutter speed, ISO, and aperture, and one
> sentence on what you had to change. Then ONE PARAGRAPH on which of the four belongs in the yearbook and
> why." Add: "Due Friday, with your PDF check-in." One slide.

### Board version (no projector)

```
SHUTTER SPEED = how long light gets in
  bigger bottom number = faster = MORE FROZEN
  faster = DARKER  (so open aperture / raise ISO)

  indoor sports .... 1/500+      THE SET (due Friday)
  outdoor sports ... 1/1000      1. FROZEN ......... 1/500+
  walking .......... 1/250       2. PANNING indoor . ~1/60
  portrait ......... 1/125       3. WATER frozen ... 1/1000+
  handheld floor ... 1/60        4. SUBJECT blurred  1/30-
  deliberate blur .. 1/30-          (brace the camera)
  panning .......... ~1/60
                                 note shutter + ISO + aperture
GYM: 1/500, aperture wide open,  + 1 sentence each
     ISO 1600-3200.              + 1 paragraph: which one
     Grainy+sharp BEATS            belongs in the book, why
     clean+blurry.
```
